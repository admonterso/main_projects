...
uint32_t convertBigEndianToLittleEndian(uint32_t bigEndianValue) {
    uint32_t littleEndianValue = 0;

    littleEndianValue |= ((bigEndianValue & 0xFF000000) >> 24);
    littleEndianValue |= ((bigEndianValue & 0x00FF0000) >> 8);
    littleEndianValue |= ((bigEndianValue & 0x0000FF00) << 8);
    littleEndianValue |= ((bigEndianValue & 0x000000FF) << 24);
...
uint32_t Flash_Write_Data1 (uint32_t StartPageAddress, uint32_t *Data, uint16_t numberofwords)
{
...
//    /* Erase the user Flash area */
//    uint32_t StartPage = GetPage(StartPageAddress);
//    uint32_t EndPageAddress = StartPageAddress + numberofwords * 4;
...
void Flash_Write_NUM (uint32_t StartSectorAddress, float Num)
{
	...
	Flash_Write_Data1 (StartSectorAddress, (uint32_t *)bytes_temp, 1);
}
